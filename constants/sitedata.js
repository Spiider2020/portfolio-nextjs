...
export const projects = [
	...
	},
	{
		...
		description:
			"This is a reverse bidding system for courses I started building a while back. What is this reverse bidding you might ask, well, in a few words the teachers were supposed to lower their fees for teaching courses if they felt they didn't get enough work. The site has fixed prices for courses witch means the lower the price for the teacher the more profit the site made. The courses are one on one. The company that I was building this for went another direction so didn't completely finish this, meaning it has no back-end. This is fully functional , the only drawback is that everything is hardcoded, so on refresh every new user,booking,setting will be lost.[p] I will explain a little bit how this works and what it does. Your can register as a Student or as a Teacher. The Student account has access to the dashboard where they can see their booked courses or book a new course. When booking the course they will go through a 4 step process where they select the course, then the date and then they will receive available(green) time-slots for that course witch will correspond to the availability of the teachers that have the course in their list. After they select the time-slot they will be prompted to play , and then they can finish the booking. The teacher what will be assigned to the course is the one with the lowest price in that time-slot, then that teacher will not be available for that timeslot anymore , the one with the second lowest price will, and so on..( basically maximizing profits ) . The Profile section is self explanatory.[p] Compared to a Student account , the Teacher account has a few extra thing in the Profile section, like adding qualification and experience , and also adding or editing the taught courses list. Dashboard has your bookings and course history where you can see the courses you taught and some basic stats.[p] The Admin account basically the same Dashboard and Profile, the only difference being that they can see all the booked courses , with a profit field as well. Admin has also a Post Course section where you can add - delete - edit the courses available on your site. On deletion the course will be removed from availability, and it will not affect already booked courses.",
